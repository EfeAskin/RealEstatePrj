...
def process_property_data(property_item):
    ...
    # UI tarafındaki değişkenleri DB kolonlarıyla eşleştirme
    property_item["net_m2"] = property_item.get("net_m2") or 0
    property_item["beds"] = property_item.get("room_count") or 0
    ...
    if "property_type" in property_item and property_item["property_type"]:
        ...
        property_item["property_type"] = str(property_type_val).capitalize()
...
async def add_property_full(
    ...
    success = db.add_full_property_to_db(agent_id, property_data, features, image_urls)
